    ...
    <div class="col-md-12">
      <div class="card mb-4">
        ...
        <div class="card-body demo-vertical-spacing demo-only-element">
          ...
          <form action="/v1/customers/{{ $data->id }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <select name="user_id" id="user_id"
                  ...
                  aria-label="Outlet Owner">
                    @foreach ($users as $outlet_owner)
                      <option value="{{ $outlet_owner->id }}" {{ ($data->user_id == $outlet_owner->id) ? 'selected' : '' }}>{{ $outlet_owner->name }}</option>
                    @endforeach
                  </select>
            ...
            <div class="form-password-toggle mb-3">
              ...
              <img src="{{ asset('storage/'.$data->photo) }}" alt="{{ $data->outlet_name }}" class="img-thumbnail mt-3" width="100">
            </div>
